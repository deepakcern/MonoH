...
f1 = ROOT.TFile.Open('punzi_significance-50.root', 'read')
f2 = ROOT.TFile.Open('pseudo_NLO_Mchi-1_Mphi-50.root', 'read')
...
h_tau1_trackEtaRel_1= f1.Get('h_tau1_trackEtaRel_1')
...
h2_tau_vertexEnergyRatio_0= f1.Get('h_tau_vertexEnergyRatio_0')
h2_tau_vertexDeltaR_0= f2.Get('h_tau_vertexDeltaR_0')
h2_tau_flightDistance2dSig_0= f1.Get('h_tau_flightDistance2dSig_0')
...
for i in range(len(hists1)):
    legend=TLegend(.43,.69,.67,.89)
    legend.SetTextSize(0.035)
    hists1[i].Draw()
    hists1[i].SetXTitle(xaxis[i])
    hists1[i].SetLineColor(2)
