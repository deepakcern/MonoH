...
filename = '../bin/limits_monoH2016.txt'
f = open(filename,"r")
MH4=array('f')
MH3=array('f')
expm2=array('f')
...
#        masspointstr = 'NLO_pseudo_mphi_'+line.rstrip().split()[0]+'_mchi_'+line.rstrip().split()[1]
    ...
    MH4.append(float(line.rstrip().split()[0]))
    MH3.append(float(line.rstrip().split()[1]))
...
g_exp2  = TGraphAsymmErrors(int(len(MH4)), MH4, expmed, errx, errx, expm2, expp2 )   ;  g_exp2.SetName("exp2")
g_exp1  = TGraphAsymmErrors(int(len(MH4)), MH4, expmed, errx, errx, expm1, expp1 )   ;  g_exp1.SetName("exp1")
g_expmed = TGraphAsymmErrors(int(len(MH4)), MH4, expmed)   ;  g_expmed.SetName("expmed")
g_obs    = TGraphAsymmErrors(int(len(MH4)), MH4, obs   )   ;  g_obs.SetName("obs")
...
f1 = TFile('monoHbb.root','RECREATE')
#g_expm2.Write()
#g_expm1.Write()
#g_expmed.Write()
#g_expp1.Write()
