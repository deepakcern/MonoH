...
f=TFile('monoHbb.root','read')
rootfilename='monoHbb.root'
exp2s =  f.Get("exp2")
...
leg = TLegend(.15, .65, .35, .890);
exp2s.GetXaxis().SetTitle("M_{a} [GeV]");
exp2s.GetXaxis().SetRangeUser(140,700)
exp2s.GetYaxis().SetRangeUser(.05,100000)
...
line = TLine(140,1,exp2s.GetXaxis().GetXmax(),1)
...
latex.DrawLatex(0.18, 0.93, "2HDM+a,   h #rightarrow b#bar{b} ")#r"                             35.9 fb^{-1}(13 TeV)");
...
latex2.DrawLatex(0.38, 0.73, "M_{A}=600 GeV, tan#beta = 1, sin#theta = .35");
